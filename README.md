...
`nftables`, `jq`, `curl` (плюс базовые `awk`/`sed`/`sort` из coreutils/gawk).
Действие `install` доставит недостающее само (`apt-get`, с фолбэком на `dnf`/`yum`).
Поставить заранее вручную (необязательно):
...
table inet filter {
    ...
    set m443_antiscanner {
        type ipv4_addr
        flags interval
        auto-merge
    }
    set m443_mobile_allow {
        type ipv4_addr
        flags interval
        auto-merge
    }
...
`flags interval` + `auto-merge` обязательны: RIPEstat отдаёт перекрывающиеся
префиксы, без `auto-merge` заливка упадёт с ошибкой `interval overlaps`.
Каждое поле сета — на отдельной строке (форма `{ … ; auto-merge; }` в одну
строку не парсится).
